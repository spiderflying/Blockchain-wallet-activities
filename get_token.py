...
data = pd.DataFrame(columns=['address','blockNumber','timestamp','hash','blockHash','txn_from','txn_to','tokenName','tokenSymbol','token_value',"gas_price",'gas_fee_eth','send','receive','confirmations','confirmed'])
...
def create_features(data):
    ...
    # print("step1")
    # df1=data_sorted.groupby(['address','timestamp','blockNumber','hash','blockHash','tokenSymbol','txn_from','txn_to'])['netflow'].sum().reset_index()
    # print(df1.columns)
    # print("step2")
    # df2=df1.groupby(['address','tokenSymbol']).cumsum().reset_index()
    # df2.rename(columns={'netflow': 'balance'}, inplace=True)
    # print(df2.columns)
    # balance=pd.concat([df1,df2],axis=1)
    # balance = balance.drop(['netflow','index'], axis=1)
    # print (balance.columns)
    # print("merge")
    # new_df=pd.merge(balance,data_sorted,"inner",on=['address','timestamp','blockNumber','hash','blockHash','tokenSymbol','txn_from','txn_to'])
...
new_df.to_csv("get_token_txn_by_address.csv",index=False)
